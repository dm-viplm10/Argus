"""Phase Strategist node — evaluates Phase 1 findings and dynamically decides next phases."""
...
def _build_findings_summary(state: dict[str, Any]) -> str:
    ...
    if facts:
        sample = facts[-15:]  # Most recent / representative
...
async def phase_strategist_node(state: dict[str, Any], *, router: ModelRouter) -> dict[str, Any]:
    """Evaluate Phase 1 findings and decide whether to add phases or proceed to synthesis."""
    writer = get_stream_writer()
    writer({"node": "phase_strategist", "status": "started"})

    findings_summary = _build_findings_summary(state)

    prompt = PHASE_STRATEGIST_SYSTEM_PROMPT.format(
    ...
    if decision.action == "add_phases" and decision.phases_to_add:
        # Append new phases to research_plan, renumber phase_number to match position
        ...
        updates["dynamic_phases"] = False  # One-shot expansion; no further dynamic phases
        # Seed pending_queries from first new phase for query_refiner to refine or use
        ...
        # Proceed to synthesis — leave state as-is; supervisor will route to synthesizer
        # (phase_complete=True, current_phase=1, max_phases=1 ⇒ current_phase >= max_phases)
